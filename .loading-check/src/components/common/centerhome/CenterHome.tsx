    ...
    <main className="min-h-screen bg-[#1f1f1f] sm:flex sm:justify-center sm:px-4 sm:py-8">
      ...
      >
        ...
        >
          ...
          {loading ? <LoadingIndicator compact label="처리 현황 요약을 불러오는 중입니다." className="h-[80px]" /> : <dl className="mt-[17px] grid grid-cols-3 text-center">
            {summaryItems.map((item) => (
              <div key={item.label}>
                <dt className="text-[12px] leading-[15px] text-[#637277]">{item.label}</dt>
                <dd className="mt-[7px] text-[22px] leading-[27px] text-[#242f33]">{error ? "—" : `${item.value}건`}</dd>
              </div>
            ))}
          </dl>}
          </div>
        ...
        <section className="absolute left-[25px] top-[611px] w-[calc(100%-50px)]" aria-labelledby="pending-title">
          ...
          <div className="ml-[13px] mt-[14px] min-h-[207px] max-w-[325px] space-y-[10px] rounded-[12px] border border-[#dbe1ca] bg-[#f4f7eb] px-[22px] py-[19px]" aria-busy={loading}>
            ...
            {!error && loading ? <LoadingIndicator label="처리 항목을 불러오는 중입니다." className="h-[169px]" /> : null}
