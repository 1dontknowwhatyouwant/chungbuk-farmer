    ...
    <main className="min-h-screen bg-[#1f1f1f] sm:flex sm:justify-center sm:px-4 sm:py-8">
      <section className={styles.screen}>
        ...
        <form className={styles.filters} onSubmit={(event) => { event.preventDefault(); void search(); }}>
          ...
          {jobsLoading || jobsError ? <div className="mt-3"><CenterFeedback loading={jobsLoading} loadingLabel="매칭할 공고 목록을 불러오는 중입니다." error={jobsError} onRetry={() => void loadPostings()} /></div> : null}
        ...
        <section className={styles.results} aria-labelledby="matching-candidates-title" aria-busy={searching}>
          ...
          <CenterFeedback loading={searching} loadingLabel="후보 도시 농부 목록을 불러오는 중입니다." error={searchError} onRetry={() => void search()} />
