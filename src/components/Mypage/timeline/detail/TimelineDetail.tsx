...
const splitLines = (value: string | string[] | null | undefined) =>
...
export default function TimelineDetail({ jobId }: { jobId: string }) {
  ...
  const checklistKey = `chungbuk-farmer-checklist-${jobId}`;

  useEffect(() => {
    try {
      const saved = window.localStorage.getItem(checklistKey);
      if (saved) setCheckedItems(JSON.parse(saved) as string[]);
    } catch { /* 손상된 로컬 값은 무시합니다. */ }
  ...
  return <main className="min-h-screen bg-[#1f1f1f] sm:flex sm:justify-center sm:px-4 sm:py-8">
    <section className="mx-auto min-h-[100svh] w-full max-w-[402px] overflow-hidden bg-white pb-8 text-[#5c5c5c] shadow-2xl">
      ...
      {error ? <p className="p-8 text-center text-[12px] text-red-500">작업 정보를 불러오지 못했습니다.</p> : <>
        ...
        <div className="mx-[43px] mt-[14px] min-h-[213px] rounded-[17px] bg-[#e4fcb5] px-6 py-[12px] shadow-[inset_0_1px_14.3px_#cfdfe8]"><p className="m-0 text-[10px] text-[#807e7e]">복장 및 준비물</p>{supplies.slice(0, 5).map((item) => <label key={item} className="flex h-[33px] items-center justify-between text-[12px]"><span className={checkedItems.includes(item) ? "line-through opacity-60" : ""}>{item}</span><input type="checkbox" checked={checkedItems.includes(item)} onChange={() => toggleChecklistItem(item)} className="h-[26px] w-[26px] appearance-none rounded-[2px] border border-[#e5e5e5] bg-white checked:bg-[#8bb84b]" /></label>)}{!supplies.length && <p className="mt-4 text-[12px] text-[#807e7e]">등록된 준비물이 없습니다.</p>}</div>
